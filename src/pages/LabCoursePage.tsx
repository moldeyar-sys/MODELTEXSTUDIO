...
import { getLabPreviewLessons, LAB_PLANNED_RESOURCES } from '../data/labCourses';
import type { LabCourseWithContent } from '../lib/labTypes';
import type { FreeMold } from '../lib/types';
import { trackLabStart, trackMicrolearningVideoStart } from '../lib/analytics';

const SITE_URL = 'https://modeltex.com.ar';
const RESOURCE_ICONS = { software: Cpu, pdf: FileDown } as const;
...
export default function LabCoursePage() {
  ...
  // Grilla de "Microlearning": datos estáticos (src/data/labCourses.ts), no Supabase —
  // ver el comentario en ese archivo sobre qué es (y qué no es) esta lista.
  const previewLessons = getLabPreviewLessons(course.slug);
  const featuredPreview =
    (progress.lastLessonSlug && previewLessons.find((l) => l.lessonSlug === progress.lastLessonSlug)) ||
    previewLessons[0];
        ...
        <div className="grid lg:grid-cols-[minmax(0,1fr)_360px] gap-8">
          <div className="space-y-6">
            ...
            <div className="card overflow-hidden p-0">
              <Link
                to={featuredPreview ? `/lab/${course.slug}/${featuredPreview.moduleSlug}/${featuredPreview.lessonSlug}` : '#'}
                onClick={() => {
                  if (!featuredPreview) return;
                  trackMicrolearningVideoStart(featuredPreview.id);
                  if (progress.completedLessonIds.length === 0) trackLabStart(course.slug);
                }}
              ...
              {featuredPreview && (
                <div className="p-4 sm:p-5">
                  <p className="text-xs font-bold uppercase tracking-wide text-primary-500">
                    {progress.completedLessonIds.length > 0 ? 'Retomar clase' : 'Primera clase'} · {featuredPreview.level}
                  </p>
                  <h2 className="font-display text-lg font-bold text-primary-900 mt-0.5">{featuredPreview.title}</h2>
                  <Link
                    to={`/lab/${course.slug}/${featuredPreview.moduleSlug}/${featuredPreview.lessonSlug}`}
                    onClick={() => {
                      trackMicrolearningVideoStart(featuredPreview.id);
                      if (progress.completedLessonIds.length === 0) trackLabStart(course.slug);
                    }}
          ...
          {/* Grilla de lecciones cortas (datos estáticos, ver src/data/labCourses.ts) */}
            ...
            <div className="card p-5 sm:p-6 lg:sticky lg:top-24 lg:max-h-[calc(100vh-7rem)] lg:overflow-y-auto">
              ...
              <div className="space-y-5">
                {previewLessons.map((l, i) => {
                  const showLevel = l.level && l.level !== lastLevel;
                  lastLevel = l.level || lastLevel;
                  return (
                    <div key={l.id}>
                      {showLevel && (
                        <p className="text-[11px] font-bold uppercase tracking-wide text-primary-500 mb-1.5">{l.level}</p>
                      )}
                      <Link
                        to={`/lab/${course.slug}/${l.moduleSlug}/${l.lessonSlug}`}
                        onClick={() => trackMicrolearningVideoStart(l.id)}
                      ...
                      >
                        ...
                        <span className="text-[11px] text-gray-400 flex-shrink-0">{l.durationMinutes} min</span>
                      </Link>
                    </div>
                  );
                })}
                {previewLessons.length === 0 && <p className="text-sm text-gray-400">Próximamente nuevas clases.</p>}
              </div>
            </div>
